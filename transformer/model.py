...
def get_non_pad_mask(seq):
    """[B, S] -> [B, S, 1], 1.0 on real events and 0.0 on PAD."""
    assert seq.dim() == 2
    return seq.ne(PAD).type(torch.float).unsqueeze(-1)

def get_attn_key_pad_mask(seq_k, seq_q):
    """Mask the PAD keys for every query position."""
    len_q = seq_q.size(1)
...
def get_subsequent_mask(seq):
    """Upper-triangular mask hiding the future from each position."""
    sz_b, len_s = seq.size()
...
class Encoder(nn.Module):
    """Masked self-attention encoder over a case prefix."""
    ...
    def __init__(
        ...
        self.register_buffer(   # temporal encoding basis
            'position_vec',
            torch.tensor(
    ...
    def temporal_enc(self, time, non_pad_mask):
        """Sinusoidal encoding of the event times: [B, S] -> [B, S, d_model]."""
    ...
    def forward(self, event_type, event_time, non_pad_mask):
        """Encode an event sequence, masking the future and the padding."""
...
class RNN_layers(nn.Module):
    """LSTM layer on top of the encoder, projected back to d_model."""
...
class Predictor(nn.Module):
    """Simple linear predictor (used by the THP-B regression heads)."""
...
class Transformer(nn.Module):
    def __init__(self,
        ...
        # z-score stats of the THP-B temporal heads; a plain attribute, so it
    ...
    def forward(self, event_type, event_time):
        """Hidden representations and head outputs: (time_pred, rt_pred, mark_logit) for baseline_act, (tie_logit, time_params, mark_logit) for mixture."""
