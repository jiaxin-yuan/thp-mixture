#!/usr/bin/env python3
"""
Regime clustering for Table~\\ref{tab:nextevent}: the binary Group A / Group B
split, justified unsupervised.

Consistent story across all three panels: there are TWO operational regimes
(Group A = high-frequency, small-vocab, THP-favourable; Group B = day-scale or
large-vocab, seq2seq-favourable), plus BPIC15-1 as an explicitly-flagged
*outlier* (shortest gaps but a 398-activity vocabulary) that the vocabulary
criterion assigns to Group B. Features (from tab:datasets):
    x1 = median inter-event time  (log10 seconds)
    x2 = activity-vocabulary size (log10 #Act)

Outputs: fig_regime_clustering.{pdf,png}
"""
import os
...
DAY = 86400.0
# Median gap and activity count of each log, as in Table 1; recompute them with
# `python prepare_uq4ppm.py --stats`.
logs = [  # name, median dt (s), #Act, hand-label (A/B)
...
cA, cB = "#1b9e77", "#7570b3"          # Group A / Group B colours
col = np.where(hand == "A", cA, cB)

# ---- silhouette: full 10 (argmax = degenerate outlier peel) vs 9 (regime) ----
ks = list(range(2, 6))
...
sil_9 = [silhouette_score(X9z, KMeans(k, n_init=25, random_state=0).fit_predict(X9z))
         for k in ks]
# k=2 on the 9 reproduces the hand A/B(core)?
...
# ---- (a) Ward dendrogram, with the k=2 and k=3 cuts marked -----------------
