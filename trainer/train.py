...
# keeps the mixture stable early on, where E[Δt]=exp(μ+σ²/2) can diverge
GRAD_CLIP = float(os.environ.get("GRAD_CLIP", "1.0"))


def compute_baseline_time_stats(loader, device):
    """Train-split means and stds (days) of Δt and remaining time, over the non-pad prediction positions."""
...
def _run_epoch_baseline(model, loader, optimiser=None, writer=None, epoch=0, phase="Training"):
    """One epoch of THP-B (train if *optimiser* is given), the loss summing the intensity MLE term, the two z-scored regression terms and the activity CE as per-event means."""
    ...
    with ctx:
        for batch_idx, batch in enumerate(
            ...
            enc_out, (time_pred, rt_pred, mark_logit) = model(event_type, event_time)
            non_pad_mask = get_non_pad_mask(event_type).squeeze(2)

            # on raw time, so unaffected by the scaling below
            event_ll, non_event_ll = Utils.log_likelihood(
                model, enc_out, event_time, event_type)
            ll_loss = -torch.sum(event_ll - non_event_ll)

            # loss on z-scored targets, MAE reported in days
            st = model.time_stats
            if st is None:
                raise RuntimeError(
                    "model.time_stats is missing; THP-B regresses z-scored "
                    "targets and needs the train-split statistics")
            ...
            loss = (ll_loss / n_ev_b + t_loss / n_pred_b
                    + rt_loss / n_pred_b + ce_loss / n_pred_b)
...
def train_model(
    ...
    """Train up to *n_epochs*, one checkpoint per validation metric (next event MAE, remaining time MAE, log-likelihood), stopping when none improves for *patience* epochs (0 disables)."""
